...
| ID | Severity | Finding |
|----|----------|---------|
| C-PHF-C1 | 🔴 CRITICAL | `get_nft_owner` / `get_nft_data_raw` are **stubs** (lines 73–93). `get_nft_data_raw` returns *empty* slices as the owner and collection. Every call to `verify_nft_account` therefore succeeds if the caller's address equals the empty slice (which no real user has, but the check is structurally wrong — `equal_slices(owner, expected_owner)` with `owner = empty slice` is a tautology for a bogus caller, and the "ownership" validation is effectively a no-op). **This contract must not be deployed as-is.** The file even admits "Note: Actual implementation requires proper TVM run_method call / This simplified version demonstrates the expected interface". **Recommendation:** implement a real `run_method` or message-based ownership check, or delete the FunC variant and rely solely on `PaymentHub.tact`. **✅ RESOLVED AT REFERENCE LEVEL (Issue #367):** the `get_nft_data_raw` placeholder is removed. Synchronous TEP-62 reads are impossible on TON, so ownership is resolved from a resolver-gated `nft_owners` registry (`op::resolve_nft_owner`, write-once), and `verify_nft_account` rejects empty/`addr_none` owners (`owner.slice_bits() >= 267`) — mirroring `nft_account_resolver.fc` (#320). The file stays non-deployable; the 0xDEAD blocker is retained as defence-in-depth (CONTRACTS-H3 / #260). Guarded by `non-production-stubs.spec.ts` and `contracts/payments/tests/payment-hub.spec.fc` (Issue #367). |
| C-PHF-C2 | 🔴 CRITICAL | No Account Locks integration. The FunC hub's `handle_internal_transfer`, `handle_merchant_payment`, and `handle_payment_received` do not consult `account-locks.fc`. A `FRAUD_LOCK` on an account does not prevent it from sending via this hub. This is the same gap documented as F-CRIT-5 in `docs/audit/FULL_SYSTEM_AUDIT.md`. Must be fixed before any mainnet deployment. **✅ RESOLVED AT REFERENCE LEVEL (Issue #367):** the hub now stores an immutable `account_locks_contract` and mirrors lock state pushed **only** by that address via `op::apply_account_lock` (the ApplyAccountLock push pattern, #363 — synchronous cross-contract reads are impossible on TON). `handle_internal_transfer` and `handle_merchant_payment` call `can_send()` before moving funds and throw `account_locked` (107) when the sender is locked, while `handle_payment_received` stays unguarded so a locked account can always still RECEIVE (invariant I6, Lock ≠ Confiscation). The file stays non-deployable; the 0xDEAD blocker is retained (CONTRACTS-H3 / #260). Guarded by `non-production-stubs.spec.ts` and `contracts/payments/tests/payment-hub.spec.fc` (Issue #367). |
| C-PHF-H1 | 🟠 HIGH | `handle_internal_transfer` does not update any balances (lines 147–162 — only emits an event). The FunC hub is purely an event emitter and does not actually move any TBC. If this contract is deployed, users will see successful events but no balance changes. Either (a) implement real jetton transfer messages to the payer's/merchant's jetton wallets, or (b) delete this contract to avoid future confusion. **✅ RESOLVED AT REFERENCE LEVEL (Issue #367):** both `handle_internal_transfer` and `handle_merchant_payment` now atomically debit the sender and credit the recipient against an internal TBC ledger (`get_balance`/`set_balance` + `save_data`), checking `insufficient_balance` (103) **before** sending so a transfer cannot create or destroy funds (invariant I4, Atomic Transfers; invariant I1). The event is emitted only after the ledger is updated. The file stays non-deployable; the 0xDEAD blocker is retained (CONTRACTS-H3 / #260). Guarded by `non-production-stubs.spec.ts` and `contracts/payments/tests/payment-hub.spec.fc` (Issue #367). |
| C-PHF-H2 | 🟠 HIGH | `save_data` writes `paused`, `blocked_accounts`, etc., but `load_data` never persists changes until `save_data` is called. `handle_internal_transfer`/`handle_merchant_payment`/`handle_payment_received` never call `save_data`, so any state mutation intended by the flag logic (there is none today, but the pattern is fragile for future changes) will silently drop. Today the `save_data` is only called from the two admin handlers, which is correct, but the omission of `save_data` elsewhere should be asserted invariantly (or the admin path should be separated into its own contract). **✅ ADDRESSED AT REFERENCE LEVEL (Issue #367):** now that the transfer handlers mutate the balance ledger (C-PHF-H1), `handle_internal_transfer` and `handle_merchant_payment` persist their changes with `save_data`, so balance mutations are no longer silently dropped. |
...
| ID | Severity | Finding |
|----|----------|---------|
| C-PHT-H1 | 🟠 HIGH | `getOrCreateAccount` (lines 442–463) **auto-creates an account with `owner = nft_address`** when one doesn't exist. The owner check at line 239 (`require(sender() == from_account.owner, ...)`) then requires that the NFT address itself sent the message — impossible in practice, so this is safe for *new* accounts, but it silently populates the accounts map from *any* incoming `TransferInternalRequest` that references an unseen NFT. Combined with `isValidAccountNFT` (which returns `true` for any pre-existing account, line 427) this creates a storage-amplification and griefing surface: an attacker can inflate the `accounts` map by referencing arbitrary addresses, and future calls will treat those inflated entries as valid. **Recommendation:** do not create accounts on read; require an explicit `InitializeAccount` from the admin (with signed ownership proof) before any account is valid. **✅ RESOLVED (Issue #371 / PC-02):** the read path no longer creates accounts. `getOrCreateAccount` is renamed `getAccountOrDefault` and made **read-only** — it returns a transient zero-balance default (`AccountState{ balance: 0, state: ACCOUNT_STATE_ACTIVE, owner: nft_address }`) without ever calling `self.accounts.set(...)`. A `TransferInternalRequest` or `GetAccountStateRequest` that references an unseen NFT can therefore no longer inflate the `accounts` map, eliminating the storage-amplification / griefing surface. Real state is persisted only by `InitializeAccount` (now create-once) and the atomic balance update inside `TransferInternalRequest`. Guarded by `non-production-stubs.spec.ts` (Issue #371). |
| C-PHT-H2 | 🟠 HIGH | `InitializeAccount` (lines 323–335) is admin-only and writes an arbitrary `owner` / `initial_balance`. Identical issue to C-MPH-C1 — the admin can mint balance. The contract's own comment admits: `// NOTE: This is a test-only function`. It must be removed (or severely gated) before production. **✅ HARDENED (Issue #371 / PC-02):** `InitializeAccount` is now **create-once** — `require(self.accounts.get(msg.nft_address) == null, "Account already initialized")` runs immediately after the `sender() == self.admin` check, so a malicious or compromised admin can no longer re-initialize a **funded** account to reassign `owner` and drain it via `TransferInternalRequest` (the I1/I3 break reported as PC-02 / #371 is closed; Tact hashes the reject to exit code `18265`). The residual ability to seed a **fresh** account with an initial balance stays test-only (the `// NOTE: This is a test-only function` comment is retained) and is tracked alongside C-MPH-C1's admin-mint removal — production balances flow through the on-chain TBC ledger, not `InitializeAccount`. Guarded by `non-production-stubs.spec.ts` and `experiments/issue-371-paymenthub-create-once/` (Issue #371). |
...
| C-PHT-M2 | 🟡 MEDIUM | `GetAccountStateRequest` (line 403) responds to any sender and materialises a new account via `getOrCreateAccount`. This is a read-only query but it writes to storage. Any querier can cause storage growth. **Recommendation:** use a pure getter (`get fun`) and remove the receiver form. **✅ PARTIALLY RESOLVED (Issue #371 / PC-02):** the storage-write side effect is eliminated — `GetAccountStateRequest` now reads through the read-only `getAccountOrDefault` helper, so an unauthenticated query can no longer materialise an account or grow the `accounts` map. Converting the receiver to a pure `get fun` (and removing the message form) remains the tracked follow-up. |
...
| X-6 | 🟡 MEDIUM | **Storage-growth on read.** `PaymentHub.getOrCreateAccount` writes on `get fun` calls and on receive of `GetAccountStateRequest`. Any caller can inflate contract storage. Make read paths pure. **✅ RESOLVED (Issue #371 / PC-02):** the read path is now pure. `getOrCreateAccount` is renamed `getAccountOrDefault` and returns a transient zero-balance default without ever calling `self.accounts.set(...)`, so neither a `get fun` lookup nor a `GetAccountStateRequest` receive mutates storage — a caller can no longer inflate the `accounts` map. Guarded by `non-production-stubs.spec.ts` (Issue #371). |
...
| T8 — Admin key compromise | All `admin:` state contracts | C-MPH-C1 (admin-mint via `SetAccountBalance`) — ✅ RESOLVED for MerchantPaymentHub (Issue #363): admin-mint handlers removed; admin can no longer mint balances or write lock state. The legitimate account-registration path is the resolver-gated, write-once `ResolveNFTOwner` (Issue #397) — only the immutable `nft_resolver`, not the admin, can register accounts |
...
- **Audit Scope (`audit/SCOPE.md`):** all contracts listed in issue #110 have been visited in this document. The following additional contracts were encountered and are either out of scope for this audit or were touched only incidentally: `RecurringPayments.tact`, `TransparencyRegistry.tact`, `PublicCollateralLookup.tact`, `governance/diamond_resolver.fc`. They should be reviewed in a follow-up audit pass. **Update (Issue #365):** the `TransparencyRegistry.tact` state-mutation paths (the six `Record*` handlers) have since been hardened — each now runs a fail-closed `sender()` check against a deployer-configured writer address, with regression tests in `contracts/governance/TransparencyRegistry.spec.ts`. See `docs/security/THREAT_MODEL.md` §4.1.5.
...
5. **Defence-in-depth pass** — address MEDIUM findings (X-3 dual encodings, X-5 composite keys, ~~X-6 storage growth~~ — X-6 RESOLVED in Issue #371 / PC-02) as a batch.
...
|---------|----------|-----------------------|---------------|
| X-1, C-CS-* (CollateralSignal `RegisterNFTOwner`) | `contracts/CollateralSignal.tact` | **RESOLVED (Issue #364).** The test-only `RegisterNFTOwner` handler (and the `deployer` field gating it) is **removed**. NFT ownership is now bound ONLY via `ResolveNFTOwner`, which requires `sender() == self.nft_resolver` — the immutable on-chain NFT Account Resolver set at `init(nft_resolver)`. The binding stays write-once (CONTRACTS-M1). The deployer can no longer register ownership (invariant I3). | None for this finding. Guarded by the `non-production-stubs.spec.ts` regression test (Issue #364) and on-chain Sandbox tests (`contracts/collateral-signal/`). |
...
| C-PHF-C1, C-PHF-C2, C-PHF-H1 (FunC `payment-hub.fc` stub) | `contracts/payments/payment-hub.fc` | **RESOLVED AT REFERENCE LEVEL (Issue #367).** All three structural gaps are fixed in the reference: (C1) the `get_nft_data_raw` placeholder is removed and ownership is resolved from a resolver-gated, write-once `nft_owners` registry with an `owner.slice_bits() >= 267` guard; (C2) lock state is mirrored via the gated `op::apply_account_lock` push and `can_send()` blocks a locked sender in both transfer handlers while `handle_payment_received` stays open (invariant I6); (H1) both transfer handlers atomically debit/credit an internal TBC ledger and check `insufficient_balance` before sending (invariant I4/I1). The `DEPLOY_BLOCKER_NOT_PRODUCTION_READY` (0xDEAD) `throw` in `recv_internal` is **deliberately retained** as defence-in-depth, because the production hub is `PaymentHub.tact` and this FunC file is permanently non-deployable (CONTRACTS-H3 / #260). The static CI gate `contracts/payment-hub/non-production-stubs.spec.ts` couples the three fixes to the blocker — it fails the build if the C1/C2/H1 hardening regresses, if the 0xDEAD constant disappears, or if the FunC reference suite `contracts/payments/tests/payment-hub.spec.fc` is missing — so the blocker cannot be removed without the integration coverage being in place. | None at the reference level for C1/C2/H1. The file remains non-deployable by design; real production funds flow through `PaymentHub.tact` and the on-chain TBC ledger. |
| C-MPH-C1 (MerchantPaymentHub `SetAccountBalance` admin mint) | `contracts/MerchantPaymentHub.tact` | **RESOLVED (Issue #363).** `SetAccountBalance` and `SetAccountState` have been **removed entirely** from the deployable production contract. The shared payment logic now lives in the `MerchantPaymentHubBase` trait; the two admin-mint / admin-register handlers exist ONLY in the non-deployable test harness `contracts/merchant-hub/test/MerchantPaymentHubHarness.tact`. A CI regression guard (`contracts/payment-hub/non-production-stubs.spec.ts`) fails the build if either handler reappears in the production source, and `scripts/deploy/deployable-contracts.ts` lists the harness as a non-production stub. In production, balances are funded by the on-chain TBC ledger/settlement flow and accounts are registered by the NFT Account Resolver via the resolver-gated, write-once `ResolveNFTOwner` handler shipped in **Issue #397** — the #363 removal left this registration path documented-but-unimplemented, so a freshly deployed hub had permanently empty `nft_owners` / `account_states` and every payment failed (audit CHECK393-M1). There is no admin-mint path. | None for the admin-mint vector. The architectural NFT-ownership source-of-truth item (`C-MPH-H1` / `X-2` — stale `nft_owners` after a *post-registration* NFT transfer) is tracked separately; #397 lands the resolver-gated binding but not the transfer-resync. |

Findings **not** addressed in this PR (and the reason):

- `X-2` (NFT ownership source-of-truth), `C-MPH-H1` (stale `nft_owners` after NFT transfer): requires a message-based `INFTResolver` and re-architecting every authority check. Architectural change — needs its own issue and tests. **Update (Issue #397):** the resolver-gated, write-once binding (the #364 pattern) now applies to `MerchantPaymentHub` too — registration comes only from the trusted `nft_resolver`, removing the admin-forge vector; the remaining piece is the post-transfer resync, which still needs the message-based resolver.
...
- `C-PHT-H1`, `C-PHT-H2`, `C-PHT-M2` (Tact `PaymentHub` admin-mint and storage growth): **storage-growth and account-overwrite half RESOLVED (Issue #371 / PC-02)** — the read path is now side-effect free (`getOrCreateAccount` → read-only `getAccountOrDefault`), so a `TransferInternalRequest` / `GetAccountStateRequest` can no longer materialise accounts (`C-PHT-H1`; the write side effect of `C-PHT-M2`), and `InitializeAccount` is now create-once, closing the re-init account-overwrite/drain vector (the hijack half of `C-PHT-H2`). The residual admin-mint-into-a-**fresh**-account (`C-PHT-H2`) still needs the same `SetAccountBalance`-style removal, and the pure-`get fun` migration of `GetAccountStateRequest` (`C-PHT-M2`) remains tracked separately.
...
- Review covered the contracts listed in issue #110 plus the immediate dependencies in `contracts/types/`, `contracts/interfaces/`, and `contracts/governance/`. It did not cover `RecurringPayments.tact`, the `TransparencyRegistry.tact` state-mutation paths, or the FunC `diamond_resolver.fc` / `PublicCollateralLookup.*`. (The `TransparencyRegistry.tact` state-mutation paths were subsequently reviewed and hardened with per-handler sender authentication under Issue #365 — see §5 scope note above and `docs/security/THREAT_MODEL.md` §4.1.5.)
