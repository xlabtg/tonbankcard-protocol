...
- Post-E3 (Issue [#134](https://github.com/xlabtg/tonbankcard-protocol/issues/134)): the on-chain `risk_authority` slice resolves to a **3-of-5 hardware multi-sig wallet** (`org.ton.contracts.multisig.v2`), not a single EOA. See [`docs/governance/RISK_AUTHORITY.md`](../../docs/governance/RISK_AUTHORITY.md) for the signer roster, fraud-detection criteria, lock procedure, and ceremony details, and [`docs/governance/FRAUD_LOCK_APPEAL.md`](../../docs/governance/FRAUD_LOCK_APPEAL.md) for the user-facing appeal procedure (7 business-day SLA, 48 h on fast-track).
- **No FunC code change is required** for the multi-sig transition. The contract guard `equal_slice_bits(sender_address, risk_authority)` on lines 212 and 229 accepts whichever address is stored in `risk_authority`. The 3-of-5 threshold is enforced by the multi-sig wallet contract itself, before it emits the `op::set_fraud_lock` / `op::clear_fraud_lock` call. The on-chain migration vehicle is the existing Issue [#96](https://github.com/xlabtg/tonbankcard-protocol/issues/96) two-phase flow: `op::propose_risk_authority` (0x4001) → 7-day timelock → `op::execute_risk_authority` (0x4002) — see lines 293–322 of `account-locks.fc`.
- Long-term: will be controlled by a fully DAO-elected Risk Committee — see [`RISK_AUTHORITY.md`](../../docs/governance/RISK_AUTHORITY.md) §8.
