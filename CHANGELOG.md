...
- **No admin-mint backdoor is reintroduced (invariant I3 / audit C-MPH-C1)**: the handler seeds **only** ownership and state, never a balance. Balance funding stays with the external on-chain TBC settlement flow; tests fund the payer's balance through a sandbox storage fixture, not a production handler.
- **Deployment dependency**: `MerchantPaymentHub`'s `init()` now takes the `AccountLocks` **and** `NFTAccountResolver` addresses; the existing deployment order in `scripts/deploy/` already places both contracts first.

### Added — MerchantPaymentHub deployable-registration test suite and resolver-gating regression guard (Issue #397)
- **Reproduction + fix tests** (`contracts/merchant-hub/merchant-payment-hub-deployable.spec.ts`): `@ton/sandbox` tests that deploy the **production** `MerchantPaymentHub` (not the harness), prove a payment fails with `ERROR_PAYER_NOT_EXISTS` / `ERROR_MERCHANT_NOT_EXISTS` before registration, register payer + merchant NFT accounts via the resolver, then run an end-to-end happy-path payment. An arbitrary sender and the deployer are both rejected by `ResolveNFTOwner` (invariant I3), and the write-once binding survives a rejected overwrite (CONTRACTS-M1).
- **Static regression guard** (`contracts/payment-hub/non-production-stubs.spec.ts`): asserts the production source keeps `MerchantPaymentHub` in the deployable map, registers NFT ownership + `ACCOUNT_STATE_ACTIVE` only through the resolver-gated `ResolveNFTOwner` handler, keeps the write-once owner guard, and never reintroduces an admin / `(test-only)` registration path — so a deployer-gated or test-only registration path reintroduced into the mainnet contract fails CI.
- **Documentation**: `audit/SMART_CONTRACTS_SECURITY_AUDIT.md` (C-MPH-C1 / C-MPH-H1 / T8 / remediation-status updated for the shipped resolver-gated registration), `docs/security/THREAT_MODEL.md` (§2.1.3 + the handler-authority inventory), `docs/governance/PARAMETERS.md` (PP-18 now documents the resolver-gated production write path, PP-19 the deliberate no-balance-path, plus an Issue #397 update note), and `scripts/deploy/README.md` + `scripts/deploy/MAINNET_RUNBOOK.md` (deployment order + e2e runbook reflect the resolver dependency).
