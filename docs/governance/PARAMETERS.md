...
| PP-7 | `proposal_registry: Address?` | `SnapshotVerifier.tact` | state, write-once | unset at init | `receive(msg: SetProposalRegistry)` — typed message carries `registry` (Issue #414) | sender = `deployer` only; write-once guard requires the slot to remain unset | **T** (write-once after deploy) |
...
| PP-14 | `accounts: map<Address, AccountState>` | `payments/PaymentHub.tact` | state | empty | **No admin initializer** — `InitializeAccount` was removed from the deployable contract by Issue #427 because it could mint an arbitrary, unbacked balance into a fresh slot. Balances may increase only through a verified TBC deposit/settlement path | **U** (owner-authorized transfers; deposit/settlement integration remains required) |
...
| PP-18 | `account_states` | `MerchantPaymentHub.tact` | state | empty | **No admin setter** — the admin `SetAccountState` was removed before mainnet (Issue #363, audit C-MPH-C1) and now exists only in the test-only `MerchantPaymentHubHarness`. The single production write path is the resolver-gated, write-once `ResolveNFTOwner` handler (Issue #397, audit CHECK393-M1), which seeds a registered account to `ACCOUNT_STATE_ACTIVE`; thereafter states change only through the `MerchantPaymentRequest` flow | **G** (resolver-gated registration → ACTIVE) / **U** (owner-driven thereafter; no admin write path) |
| PP-19 | `account_balances` | `MerchantPaymentHub.tact` | state | empty | **No production funding path** — `SetAccountBalance` was removed before mainnet (Issue #363, audit C-MPH-C1, admin-mint backdoor), and balance seeding exists only in the non-deployable test harness. Production code can credit a merchant only after successfully debiting a payer, while every fresh payer starts at zero; payments therefore fail with `ERROR_INSUFFICIENT_BALANCE` (Issue #414). A non-custodial deposit/settlement mechanism requires a dedicated protocol-economics decision | **UNRESOLVED / blocks production liveness** |
...
> **Updates (Issues #397, #414):** `MerchantPaymentHub` now has the resolver-gated, write-once account-registration path that #363 documented but never shipped. It binds `nft_owners` and seeds `account_states` to `ACCOUNT_STATE_ACTIVE` (PP-18), closing the `ERROR_PAYER_NOT_EXISTS` / `ERROR_MERCHANT_NOT_EXISTS` regression. Registration deliberately does not mint a balance, but no external funding handler exists either: a registered payer remains at zero and cannot pass the balance check. Funding therefore remains unresolved under PP-19 until a non-custodial mechanism is designed and reviewed.
