...
> **Update (Issue #397):** `MerchantPaymentHub` gains the account-registration path that #363 documented but never shipped. The contract now stores an immutable `nft_resolver` (set once at `init()`, alongside `account_locks_contract`) and registers accounts exclusively through the resolver-gated, write-once `ResolveNFTOwner` handler — `require(sender() == self.nft_resolver, ...)` plus `require(self.nft_owners.get(msg.nft_address) == null, ...)` — which binds `nft_owners` and seeds `account_states` to `ACCOUNT_STATE_ACTIVE` (PP-18). This mirrors the CollateralSignal remediation (PP-36 / #364) and closes the regression where a freshly deployed hub had permanently empty `nft_owners` / `account_states` and every payment failed with `ERROR_PAYER_NOT_EXISTS` / `ERROR_MERCHANT_NOT_EXISTS` (audit CHECK393-M1). Balance funding is **not** part of this path (PP-19) — it stays with the external on-chain TBC settlement flow, preserving invariant I3.
