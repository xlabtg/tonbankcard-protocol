...
| PP-19 | `account_balances` | `MerchantPaymentHub.tact` | state | empty | `TBCDeposit` credits a registered NFT only when sent by immutable `tbc_settlement`; a globally unique `deposit_id` is consumed exactly once. `SetAccountBalance` remains test-only and the admin has no balance mutation path (Issue #428) | **U** (verified settlement provenance + replay protection; owner-authorized spending) |
...
> **Updates (Issues #397, #428):** `MerchantPaymentHub` registers accounts through resolver-gated, write-once `ResolveNFTOwner`, then accepts TBC credits only from the immutable settlement contract through replay-protected `TBCDeposit`. Registration cannot mint, the admin cannot mutate balances, and spending remains NFT-owner-authorized.
