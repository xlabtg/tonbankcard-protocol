...
| ~~`InitializeAccount` lets admin create or overwrite owner/balance~~ | `PaymentHub.tact` | ~~HIGH~~ | **RESOLVED (Issues #371, #427)** — the account read path is side-effect free and the test/setup initializer has been removed from the deployable contract, eliminating both overwrite and fresh-slot admin minting |
